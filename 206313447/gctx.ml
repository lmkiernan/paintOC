...
(*I added thickness as a type within the gctx type*)

type thickness = {t: int}

let thick : thickness = {t=15}
...
type gctx = {
  ...
  thickness: thickness; (*added thickness*)
  }
...
let with_color (g: gctx) (c: color) : gctx =
  ...
  (*I added with_thickness to mimmick the color functionality*)
...
let set_graphics_state (gc: gctx) : unit =
  ...
  Graphics.set_line_width (thickness.t)
...
let rec draw_points (g: gctx) (p1: position list): unit =
  set_graphics_state g;
  begin match p1 with 
  |[] -> ()
  |hd:: tl -> let (x, y) = ocaml_coords g hd in 
  Graphics.moveto x y;
  Graphics.plot x y; draw_points g tl
  end
