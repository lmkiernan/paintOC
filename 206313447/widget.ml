...
(** The vpair widget lays out two widgets vertically, aligned at their
    ...
    I had it so that it translates one widget (the first) on top of the other 
    and both are treated as completely separate in terms of functionality
...
let make_controller (v: 'a) : 'a value_controller = 
    let vval = {contents = v} in
    let llist = {contents = []} in
    {
    add_change_listener = (fun (f: 'a -> unit) -> llist.contents <- 
    f :: llist.contents);
    get_value = (fun () -> vval.contents);
    change_value = (fun (x: 'a) -> vval.contents <- x;
    ignore (List.fold_right (fun f acc -> (f x) :: acc) !llist []))
    }
...
(*I created the checkbox by attaching a visual widget to a value controller 
to visually display whenever a option is checked on or off*)

let checkbox (init: bool) (s: string) : widget * bool value_controller =
let first : bool value_controller = make_controller init in
let (w1, lc) = label s in
let rect : widget = {
  repaint = (fun (g: Gctx.gctx) -> if first.get_value () 
  
  then(Gctx.draw_rect g (0,0) (20,20);
 Gctx.draw_line g (0,20) (20,0); 
 Gctx.draw_line g (20, 20) (0,0);) 

 else (Gctx.draw_rect g (0,0) (20,20)););


 handle = (fun _ _ -> ());
 size = (fun () -> (20,20))
} in 
let (w :widget), (nc: notifier_controller) = notifier (hlist 
[w1; space(10, 10); rect]) in 
nc.add_event_listener (mouseclick_listener (fun () ->
  first.change_value (not(first.get_value ()));));
  (w , first)
...
(*this creates the slider widget with the repaint, handle, and size components
after defining each of the components of my widget I implement it into a label
and return the widget and the value controller*)

let slider (s : string) (c : Gctx.color) : widget * int value_controller =
let first = make_controller 80 in 
let widge : widget =
{
repaint = (fun (g: Gctx.gctx) ->
Gctx.draw_rect g (0, 0) (80, 15);
Gctx.fill_rect (Gctx.with_color g c) (0, 0) (first.get_value(), 15););

handle = (fun (g : Gctx.gctx) (e : Gctx.event) ->
let pos = Gctx.event_pos e g in 
if event_within g e (80, 15) && (Gctx.event_type e = MouseDown 
|| Gctx.event_type e = MouseDrag) then
first.change_value (fst pos););


size = (fun () -> (80, 15))
} in 
let label = fst (label s) in 
let lab_full = hpair label widge in 
(lab_full, first)
